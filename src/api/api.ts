...
import { availableKey, citiesKey, countriesKey, directionCashKey, exchangersKey } from "../assets/consts";
import { City, Countrie, DirectionCash } from "../model";
...
export const useFetchCashCountries = () => {
  const fetchExchangers = async () => (await $host.get<Countrie[]>(`/api/cash/countries`)).data
  const queryResult = useQuery({
...
export const useFetchCashAvailableCities = ({ country }: ReqFetchAvailableCitiesDto) => {
  const fetchExchangers = async () => (await $host.get<City[]>(`/api/cash/available_cities?country=${country}`)).data
  const queryResult = useQuery({
...
export const useFetchCashAvailableValutes = ({ base, city }: ReqFetchAvailableValutesDto) => {
  const fetchExchangers = async () => (await $host.get<Categories>(`/api/cash/available_valutes?city=${city}&base=${base}`)).data
  const queryResult = useQuery({
...
export const useFetchCashDirections = ({ city, valute_from, valute_to }: ReqFetchDirectionsDto) => {
  const fetchExchangers = async () => (await $host.get<DirectionCash[]>(`/api/cash/directions?city=${city}&valute_from=${valute_from}&valute_to=${valute_to}`)).data
  const queryResult = useQuery({
